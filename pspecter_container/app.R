    ...
    sidebarLayout(sidebarPanel(
      bsCollapse(multiple = T, open = list("Preselected Parameters"),
       ...
       bsCollapsePanel("Dynamic Modification Search", uiOutput("VPselect"), 
         selectInput("VPmaxmod", "Maximum Number of Modifications", choices = c("1", "2"), selected = "1"),
...
server <- function(input, output, session) {
  ...
  revals <- reactiveValues(testSeq = NULL, imgData = list(), 
              PTMs = NULL, PTMList = list(), PTMdf = NULL, PTMread = 0,
  ...
  Desc <- fread(file.path("Server", "Pop_Up_Functions", "Function_Descriptions.csv")) %>% na.omit() %>% data.frame()
